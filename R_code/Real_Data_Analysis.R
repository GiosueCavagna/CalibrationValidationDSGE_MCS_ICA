...
#> qchisq(0.999999999999999980000000000009999,iM-1)
#[1] 80.92313
#> qchisq(0.99999999999999998000000000000999,iM-1)
#[1] 79.50227
...
Simul_logY=readMat("~/Documents/CalibrationValidationDSGE_MCS_ICA/Matlab_code/Simulated_Data/Simul_logY.mat") #readMat("Data/Simul_logY.mat") #
Simul_Pi=readMat("~/Documents/CalibrationValidationDSGE_MCS_ICA/Matlab_code/Simulated_Data/Simul_Pi.mat") #readMat("Data/Simul_PI.mat") #
...
for(ii in 1:ncp){
  ...
  if (!( is.nan(Simul_logY$Simul.logY[tau,nmr,ii])      | 
         abs(Simul_logY$Simul.logY[tau,nmr,ii])>10e+2   |
         abs(Simul_logY$Simul.logY[tau,nmr,ii])<10e-14  ) ){ 
    D[[i]] = list()
    D[[i]]$list = list()
    for (j in 1:nmr){
...
counter=length(Mres)
Mres=unique(Mres)
counter=counter-length(Mres)

#I remove the problematic term
ncp=ncp-counter
